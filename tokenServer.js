...
exports.TokenProcess = () => {
  ...
  server.post('/directline/token', (req, res) => {
    const userId = (req.body && req.body.id)
      ? req.body.id : `dl_${ Date.now() + Math.random().toString(36) }`;
    const options = {
    ...
    request.post(options, (error, response, body) => {
      ...
      } else if (response.statusCode >= 400 && response.statusCode < 500) res.send({ statusCode: response.statusCode, error: "Direct Line's generate token API returned error" });
  ...
  server.post('/directline/refresh', (req, res) => {
    ...
    request.post(options, (error, response, body) => {
      ...
      } else if (response.statusCode >= 400 && response.statusCode < 500) res.send({ statusCode: response.statusCode, error: "Direct Line's token refresh API returned error" });
      else if (response.statusCode >= 500) {
        res.status(response.statusCode);
  ...
  server.post('/directline/conversations', (req, res) => {
    ...
    request.post(options, async (error, response, body) => {
      ...
      } else if (response.statusCode >= 400 && response.statusCode < 500) res.send({ statusCode: response.statusCode, error: "Direct Line's conversations API returned error" });
      else if (response.statusCode >= 500) {
        res.status(response.statusCode);
  ...
  server.post('/directline/reconnect', (req, res) => {
    ...
    request.get(options, (error, response, body) => {
      ...
      } else if (response.statusCode >= 400 && response.statusCode < 500) res.send({ statusCode: response.statusCode, error: "Direct Line's reconnect API returned error" });
      else if (response.statusCode >= 500) {
        res.status(response.statusCode);
  ...
  server.post('/speechservices/token', async (req, res) => {
    ...
    request.post(options, (error, response, body) => {
      if (!error && response.statusCode < 300) {
        const json = { region: process.env.SPEECH_SERVICES_REGION, authorizationToken: body };
        res.send({
          authorizationToken: json.authorizationToken,
          region: json.region,
        });
        logger(`\nSomeone requested a speech token...(${ response.statusCode })\n`);
      } else if (response.statusCode >= 400 && response.statusCode < 500) res.send({ statusCode: response.statusCode, error: "CS's speech token API returned error" });
      else if (response.statusCode >= 500) {
        res.status(response.statusCode);
